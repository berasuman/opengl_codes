...
GLint n=0,o=0,p=0,q=0,r=0,s=0,t=0,u=0,v=0,w=0;// rmn unused 
...
GLint n1=0,n2=0,n3=0,n4=0,n5=0,n6=0,n7=0,n8=0,n9=0;//new variables created for bts
//Functions Prototype
void zoom();
...
void ground(){
    ...
    //Objects on Earth
    glBegin(GL_QUADS);       //Ground on Earth 
...
void groundmars(){
        ...
        glBegin(GL_POLYGON);
            glVertex2d(410+mrx,200+100+mry);
            glVertex2d(410+mrx,230+100+mry);
            glVertex2d(470+mrx,230+100+mry);
            glVertex2d(500+mrx,200+100+mry);
        glEnd();
...
{
     glLineWidth(5.0);
     glBegin(GL_LINES);         //Top-down Aproach
     ...
     glEnd();
         glBegin(GL_POLYGON);// todo head replace with ellipse
    glColor3f(0,0,0);
    int triamount=20;
...
void display()
{
    ...
    if(gtrack>=-0.5 && gtrack<0.8){
        glEnable(GL_BLEND);
        glBlendFunc(0.6,0.2);
    }
    if(gtrack>0.80 && gtrack<0.90)
        glDisable(GL_BLEND);
    if(gtrack>=0.90 && gtrack<0.91){
        glEnable(GL_BLEND);
        glBlendFunc(0.6,0.2);
    }
    if(gtrack>1.2 && gtrack<1.80)
        glDisable(GL_BLEND);
...
void zoom()
{
    ...
    if(gtrack>34 && gtrack<44)
        glViewport(-3850,-1100,25600,14400);
    if(gtrack>45 && gtrack <46)
        glViewport(-5000,-2800,51200,28800);
    if(gtrack>46 && gtrack<50){
        u+=3;
    	glViewport(-5000-(u/2),-2800,51200,28800);
    }
}
void bts(){
    if(gtrack>44){
    glClearColor(0.1,0.4,0.2,0.0);
    ...
    glBegin(GL_LINES);         //Top-down Aproach
        ...
        glVertex2i(-10+n1,230);       //torso
        glVertex2i(-10+n1,330);
        glVertex2i(-10+n1,310);        //Obvious left hand
        glVertex2i(-60+n1+n2,230);
        glVertex2i(-10+n1,310);        //right hand
        glVertex2i(40-n2+n1,230);
        glVertex2i(-10+n1,230);        //Assumed left leg
        glVertex2i(-60+n1+n2,150);
        glVertex2i(-10+n1,230);        //right leg
        glVertex2i(40-n2+n1,150);
    glEnd();
    glBegin(GL_LINES);         //Top-down Aproach
        glColor3f(0.1,0.0,0.1);
        glVertex2i(300,230);       //torso
        glVertex2i(300,330);
        glVertex2i(300,310);        //Obvious left hand
        glVertex2i(250+n2,230);
        glVertex2i(300,310);        //right hand
        glVertex2i(350-n2,230);
        glVertex2i(300,230);        //Assumed left leg
        glVertex2i(250+n2,150);
        glVertex2i(300,230);        //right leg
        glVertex2i(350-n2,150);
    glEnd();
    glBegin(GL_LINES);         //Top-down Aproach
        glColor3f(0.1,0.0,0.1);
        glVertex2i(600,230);       //torso
        glVertex2i(600,330);
        glVertex2i(600,310);        //right hand
        glVertex2i(650-n2,230);
        glVertex2i(600,230);        //right leg
        glVertex2i(650-n2,150);
    glEnd();
    glBegin(GL_LINES);         //Top-down Aproach
        glColor3f(0.1,0.0,0.1);
        glVertex2i(900,230);       //torso
        glVertex2i(900,330);
        glVertex2i(900,310);        //Obvious left hand
        glVertex2i(850+n2,230);
        glVertex2i(900,230);        //Assumed left leg
        glVertex2i(850+n2,150);
    glEnd();
    glBegin(GL_LINES);         //Top-down Aproach
        glColor3f(0.1,0.0,0.1);
        glVertex2i(300+900,230);       //torso
        glVertex2i(300+900,330);
        glVertex2i(300+900,310);        //right hand
        glVertex2i(300+950-n2,230);
        glVertex2i(300+900,230);        //Assumed left leg
        glVertex2i(300+850+n2,150);
    glEnd();
    //Evolution of rocket ^_^
    glColor3f(1.0,0.0,0.0);
    glBegin(GL_POLYGON);
        glVertex2d(500+900,200);
        glVertex2d(500+900,400);
        glVertex2d(500+1000,400);
        glVertex2d(500+1000,200);
    glEnd();
    glBegin(GL_TRIANGLES);
        glVertex2d(500+900,410);
        glVertex2d(500+1000,410);
        glVertex2d(500+950,560);
    glEnd();
    n1+=2;
        n2+=2.5;
        if(n2==100)
...
void rand_generate_for_stars()// Stars are Created here.
{
	int i1,i2;
	srand(time(0));
	for(i1=0;i1<110;i1++)
...
void story()
{
    ...
    if(gtrack>-9 && gtrack<-7.5){
        ...
        if(gtrack>-8.5)
            glColor3f(((rand()%(110))/100),0.0,0.0);
        int l=strlen(c2);
  	    for (int i=0;i<l;i++)
    ...
    if(gtrack>-7.5 && gtrack<-6.5){
        char c3[]="In a Fictional Timeline";
        char c5[]=".";
        glPushMatrix();
    ...
    if(gtrack>-6.5 && gtrack<-4.5){
        char c4[]="A Man Decides to Go to Mars";
        char c5[]=".";
        int l2=strlen(c4);
        glPushMatrix();
        glScalef(0.6f,0.6f,0);
    ...
    if(gtrack>-4.2 && gtrack<-3.2){
        char c6[]="*This Animation has no Sound";
        char c7[]=".";
        int l2=strlen(c6);
        glPushMatrix();
        glScalef(0.6f,0.6f,0);
        glTranslatef(0,-200,0);
        glColor3f(0.1,0.0,((rand()%(110))));
        for (int i=0;i<l2;i++)
            glutStrokeCharacter(GLUT_STROKE_MONO_ROMAN , c6[i]);
        if(gtrack>-3.8&& gtrack<-3.4)
            for(int k=0;k<1;k++)
            glutStrokeCharacter(GLUT_STROKE_MONO_ROMAN , c7[i]);
        glPopMatrix(); 
    }
    if(gtrack>-3.2 && gtrack<-2.0){
        char c8[]="It's still a mystery how did he entered the Rocket.";
        char c9[]=".";
        int l2=strlen(c8);
        glPushMatrix();
        glScalef(0.3f,0.6f,0);
        glTranslatef(-150,-200,0);
        glColor3f(0.1,((rand()%(110))),0.0);
        if(q<=l2){
            q+=(int)1.0;
            for (int i=0;i<q;i++)
            glutStrokeCharacter(GLUT_STROKE_MONO_ROMAN , c8[i]);
            for(int k=0;k<1;k++)
                glutStrokeCharacter(GLUT_STROKE_MONO_ROMAN , c9[i]);
        }
        glPopMatrix(); 
    }
    if(gtrack>-2.2 && gtrack<-0.5){
        char c8[]="It's still a mystery how he entered the Rocket.";
        char c9[]=".";
        int l2=strlen(c8);
        glPushMatrix();
        glScalef(0.3f,0.6f,0);
    ...
    if(gtrack>37  &&  gtrack<42){
        
        if(gtrack>37){
            char c18[]="Computer Graphics with OpenGL 3/e Hearn and Baker";
            glPushMatrix();
            glColor3f(((rand()%(101))/100),(1.0-((rand()%(101))/100)),0.0);
            if(gtrack>37 && gtrack <41.5){
                glTranslatef(-350,(37-gtrack)*100,0);
                t1=(37-gtrack)*100;
            }
            else
                glTranslatef(-350,t1,0);
                glScalef(0.6f,0.6f,0);
                int l=strlen(c18);
  	            for (int i=0;i<l;i++)
        ...
        if(gtrack>39){
            char c19[]="References: ";
            glPushMatrix();
            glColor3f(0.0,((rand()%(101))/100),(1-((rand()%(101))/100)));
            if(gtrack>39 && gtrack <41){
    ...
    if(gtrack>42  && gtrack<44){
        v+=0.005;
        char c33[]="Behing the Scenes";
        glPushMatrix();
        float hold=1.0-v;
        //printf("%d",hold);
        if(hold>0.1)
            glColor3f(1.0-v,1.0-v,1.0-v);
        else
            glColor3f(0.1,0.1,0.1);
            
        glTranslatef(-230,-80,0);
	    glScalef(1.7f,1.7f,0);
        int l=strlen(c33);
  	    for (int i=0;i<l;i++)
            glutStrokeCharacter(GLUT_STROKE_ROMAN , c33[i]);
        glPopMatrix();
    }
}
//Main Function
int main(int argc,char** argv)
{
    ...
    glutCreateWindow("team @nameless");
    init();//	Initialization of Once time Things
